...
function withTextFormat(Component) {
  return class WithTextFormat extends React.Component {
    static propTypes = {
    ...
    handleOnChange = event => {
      ...
      if (
        !this.isKeyAllowed(unformattedValue) ||
        triggerType === 'Undo' ||
        triggerType === 'Redo'
      ) {
        return;
    ...
    handleDelete = (unformattedValue, action) => {
      ...
      if (selectionStart === selectionEnd) {
        let startPosition =
          selectionStart - getCountOfSymbolsInSelection(0, selectionStart, pattern);
        let toDelete = 0;
    ...
    render() {
      const { pattern, ...otherProps } = this.props;
