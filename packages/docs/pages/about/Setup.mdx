...
import { mapLocale } from "@transferwise/components/build/es/polyfill/common/locale";

import appTranslations from '...';

export default function App() {
  const locale = mapLocale(global.locale);
...
const App = () => (
  <TranslationProvider messages={appTranslations} language={global.locale}>
    <Provider i18n={{ locale, messages: componentsTranslations[locale] }}>
      ..
    </Provider>
  </TranslationProvider>
);
