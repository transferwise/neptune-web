...
import {
  ...
  isLocaleValid,
  // Verifies and adjusts locale (replace `_` with `-`) returns null if locale is unrecognized by {Intl.Locale}
  adjustLocale,
  // * Provides corresponding country code (iso2) for provided locale if the value is invalid returns null
  getCountryFromLocale,
  // Provides corresponding lang (iso2) for provided locale if locale is invalid return null
  getLangFromLocale,
...
const App = () => (
  <TranslationProvider messages={appTranslations} language={global.locale}>
    <Provider i18n={{ locale, messages: componentsTranslations[lang] }}>
      ..
    </Provider>
  </TranslationProvider>
);
