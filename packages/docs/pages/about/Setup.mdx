...
import { adjustLocale, getLangFromLocale } from "@transferwise/components/build/es/polyfill/common/locale";

import appTranslations from '...';

export default function App() {
  const locale = adjustLocale(global.locale);
  const lang = getLangFromLocale(locale);
  const messages = { ...componentsTranslations[lang], ...appTranslations[lang] };
  return (
...
**Note:** we recommend map you locale code to ours before passing it into `Provider` and fetching translations, you can use our internal API for this:
...
import {
  ...
  // `en`
  DEFAULT_LANG,
  // array of strings of supported languages
  SUPPORTED_LANGUAGES,
  // Verifies whether provided local is valid using JS natives (Intl)
  isLocaleValid,
  // Verifies and adjusts locale (replace `_` with `-`) returns null if locale is unrecognized by Intl
  adjustLocale,
  // * Provides corresponding country code (iso2) for provided locale if the value is invalid returns null
  getCountryFromLocale,
  // Provides corresponding lang code (iso2) for provided locale if the value is invalid returns null
  getLangFromLocale,
...
**Note:** `Provider` component uses react-intl which creates a context using [`RawIntlProvider`](https://formatjs.io/docs/react-intl/components/#rawintlprovider).
If your code already uses react-intl and its providers (e.g `RawIntlProvider` or `IntlProvider`) replace them with `Provider` passing all translations objects (`@transferwise/components` translation files, your app translation files, etc) there
(Translations can be combined using rest operator (as shown on above example) or using [`formatjs compile`](https://formatjs.io/docs/guides/distribute-libraries#declaring-with-a-convention)).
In case your code uses some other framework / library then setup should be handled separately, example with retranslate library:
...
const lang = getLangFromLocale(global.locale);

const App = () => (
  <TranslationProvider messages={appTranslations} language={global.locale}>
    <Provider i18n={{ locale: global.locale, messages: componentsTranslations[lang] }}>
      ..
    </Provider>
  </TranslationProvider>
);
